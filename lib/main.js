...
module.exports = function(access_token) {
  ...
  resources.changes.watch = function(channel_id, callback_address, token, ttl, params, cb) {
	...
	Childern::
		...
		For Children Resource details, see below about Resource representations.
	
		Parameters: 
		PropertyName Value		Description	Notes
	...
	*/
	...
	resources.children = function(folderId) {
		return {
			list: function(params, cb) {
				var p = extract_params(undefined, params, cb);
				var uri = base_uri + '/files/' + folderId + '/children';
				console.log(uri);
				return make_request(p).get(uri, p.cb);
			}
		}
	}
